...
const glassTile =
  'home-glass-tile render-contained relative overflow-hidden rounded-[20px] border border-dashed border-border-muted/22 bg-[#f7f8ef] shadow-[0_12px_34px_rgba(31,43,18,0.10)] transition-all duration-300 dark:border-border-accent/20 dark:bg-card dark:shadow-[0_16px_38px_rgba(0,0,0,0.42)]'
const softKicker =
...
const fieldGlass =
  'field h-[58px] appearance-none !rounded-[20px] !bg-card px-4 !text-text !shadow-[inset_0_0_0_1px_rgba(23,33,19,0.15)] focus:ring-2 focus:ring-primary dark:!bg-[#11160e] dark:!text-[#f4f7e9] dark:!shadow-[inset_0_0_0_1px_rgba(213,230,169,0.15)] focus:ring-primary'
const iconBubble =
    ...
    <>
        ...
        <div className="relative z-10 flex flex-col gap-6">
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <h2 className="mt-4 font-montserrat text-2xl font-bold text-text">Monte um duelo limpo</h2>
          ...
          {availableOpponents.length === 0 ? (
            <div className="overflow-hidden rounded-[20px] border border-dashed border-border-muted/22 bg-[#f7f8ef] px-4 py-4 text-sm font-semibold text-text-muted shadow-sm dark:border-border-accent/20 dark:bg-card">
            ...
            <>
              <div className={`${glassTile} p-4`}>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <p className="text-sm font-black text-text">Rivais online</p>
                    <p className="mt-1 text-xs text-text-subtle">
                      Selecione alguém disponível para iniciar o pareamento.
                    </p>
                  </div>
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-card dark:bg-card text-primary shadow-sm ring-1 ring-[#172113]/15 dark:ring-[#d5e6a9]/15">
                  ...
                  {availableOpponents.map((user) => {
                    ...
                    return (
                      <button
                        ...
                        aria-pressed={active}
                        className={`flex items-center justify-between gap-3 overflow-hidden rounded-[20px] border px-4 py-3 text-left shadow-sm ${ active ? 'border-primary/30 dark:border-primary/30 bg-primary/10 text-primary' : 'border-border-muted/18 bg-[#f7f8ef] text-text-muted hover:border-primary/30 hover:bg-card dark:border-border-accent/20 dark:bg-card dark:text-text-muted dark:hover:border-primary/30' }`}
                      >
                        <span className="flex min-w-0 items-center gap-3">
                          <span className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-black text-on-primary shadow-sm">
                            {user.username.charAt(0).toUpperCase()}
                            <span className="absolute -right-0.5 -top-0.5 h-3 w-3 rounded-full border-2 border-[#fbfcf2] bg-primary dark:border-[#11160e]" />
                          </span>
                          <span className="min-w-0">
                            <span className="block truncate text-sm font-bold text-text">
                              {user.username}
                            </span>
                            <span className="mt-0.5 block text-[11px] uppercase tracking-[0.14em] text-text-subtle">
                              {user.role === 'admin' ? 'admin online' : 'online agora'}
                            </span>
                          </span>
                        </span>
                        <span
                          className={`shrink-0 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] ${ active ? 'bg-primary text-white ' : 'bg-primary/10 text-primary' }`}
                        >
              ...
              <div ref={formRef} className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <label className="ml-1 text-[10px] font-bold uppercase tracking-[0.18em] text-text-subtle">
                    Oponente
                  </label>
                  <select
                    ...
                    {GAME_TYPES.map((game) => {
                      ...
                      return (
                        <button
                          ...
                          aria-pressed={active}
                          className={`flex min-h-[58px] items-center gap-3 overflow-hidden rounded-[20px] border px-3 text-left shadow-sm ${ active ? 'border-primary/30 dark:border-primary/30 bg-primary/10 text-primary' : 'border-border-muted/10 dark:border-border-accent/10 bg-[#f7f8ef] dark:bg-card text-text-muted dark:text-text-muted hover:border-primary/30 dark:hover:border-primary/30 hover:bg-card dark:hover:bg-[#11160e]' }`}
                        >
              ...
              <div className="flex justify-end">
                <button
                  ...
                  className="inline-flex min-h-12 min-w-[220px] items-center justify-center gap-2 rounded-full bg-primary px-5 py-4 font-montserrat text-sm font-bold text-on-primary shadow-[0_10px_22px_rgba(24,59,22,0.22)] transition-colors hover:bg-primary-dark disabled:cursor-not-allowed disabled:opacity-55"
                >
                  {loading ? 'Criando...' : (
                    <>
                      <Send className="h-5 w-5" strokeWidth={2.3} />
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            <div
              className={`flex items-center gap-3 rounded-[20px] px-6 py-4 text-sm font-semibold shadow-[0_24px_60px_rgba(27,28,24,0.16)] ${ toast.type === 'success' ? 'bg-primary text-on-primary bg-primary ' : 'bg-red-700 text-white' }`}
            >
