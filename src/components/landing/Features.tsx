...
export default function Features() {
  return (
    <section id="recursos" className="scroll-mt-20 bg-transparent py-16 sm:py-20 lg:py-24">
      <div className="mx-auto w-full max-w-[var(--page-width)] px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
          <div>
            <p className="inline-flex rounded-full border border-border-muted/18 bg-primary-container px-3 py-1 text-sm font-black uppercase tracking-[0.12em] text-primary">Recursos</p>
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((feature) => {
            ...
            return (
              <article
                key={feature.title}
                className="scroll-reveal rounded-[20px] border border-dashed border-border-muted/22 bg-[#F4F1EA] p-6 shadow-[0_12px_34px_rgba(28, 25, 21,0.10)] transition-transform hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(28, 25, 21,0.13)]"
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary-container text-primary">
                  <Icon className="h-6 w-6" strokeWidth={2.2} />
                </div>
                <h3 className="mt-5 text-xl font-bold tracking-normal text-text">{feature.title}</h3>
