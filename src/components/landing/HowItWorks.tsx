...
export default function HowItWorks() {
  return (
    <section id="como-funciona" className="scroll-mt-20 bg-transparent py-16 sm:py-20 lg:py-24">
      <div className="mx-auto w-full max-w-[var(--page-width)] px-4 sm:px-6 lg:px-8">
        <div className="max-w-2xl">
          <p className="inline-flex rounded-full border border-border-muted/18 bg-primary-container px-3 py-1 text-sm font-black uppercase tracking-[0.12em] text-primary">Como funciona</p>
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-3">
          {steps.map((step, index) => {
            ...
            return (
              <article key={step.title} className="scroll-reveal rounded-[20px] border border-dashed border-border-muted/22 bg-[#F4F1EA] p-6 shadow-[0_12px_34px_rgba(28, 25, 21,0.10)]">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary-container text-primary">
                  <Icon className="h-6 w-6" strokeWidth={2.3} />
                </div>
                <p className="mt-6 text-sm font-black uppercase tracking-[0.1em] text-text-subtle">Passo {index + 1}</p>
                <h3 className="mt-2 text-xl font-bold tracking-normal text-text">{step.title}</h3>
