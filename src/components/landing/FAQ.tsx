...
export default function FAQ() {
  return (
    <section id="duvidas" className="scroll-mt-20 border-t border-border-muted/14 bg-transparent py-16 sm:py-20 lg:py-24">
      <div className="mx-auto grid w-full max-w-[var(--page-width)] gap-10 px-4 sm:px-6 lg:grid-cols-[0.75fr_1.25fr] lg:px-8">
        <div className="max-w-xl">
          <p className="inline-flex rounded-full border border-border-muted/18 bg-primary-container px-3 py-1 text-sm font-black uppercase tracking-[0.12em] text-primary">
            Dúvidas frequentes
          </p>
        ...
        <div className="space-y-3">
          {questions.map((item) => (
            <details
              key={item.question}
              className="scroll-fade group rounded-[18px] border border-border-muted/18 bg-[#F4F1EA] px-5 py-4 shadow-[0_8px_24px_rgba(28, 25, 21,0.07)]"
            >
              ...
              <p className="mt-3 max-w-3xl pr-8 leading-7 text-text-muted">
                {item.answer}
              </p>
            </details>
          ))}
