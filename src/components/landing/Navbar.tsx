...
export default function Navbar() {
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-border-muted/14 bg-card/90 backdrop-blur-xl">
      <nav
      ...
      >
        <Link href="/" className="flex min-w-0 flex-1 items-center gap-2 sm:gap-3" aria-label="Kivora English">
          <Image
            ...
            priority
          />
          <span className="hidden min-w-0 truncate font-[family:var(--font-display)] text-base font-bold tracking-normal text-text min-[390px]:inline min-[390px]:text-lg sm:text-xl">
            Kivora English
          </span>
        </Link>
        ...
        <div className="hidden items-center gap-7 lg:flex">
          {navigationLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-sm font-bold tracking-normal text-text-muted transition-colors hover:text-primary"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
          <Link
            href="/login"
            className="inline-flex h-10 items-center justify-center rounded-full px-2 text-xs font-bold text-primary transition-colors hover:bg-primary-container sm:h-11 sm:px-4 sm:text-sm"
          >
            Entrar
          </Link>
          <Link
