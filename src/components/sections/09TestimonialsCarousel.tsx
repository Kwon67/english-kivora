            ...
            {testimonials.map((testimonial, testimonialIndex) => {
              ...
              return (
                <article
                  ...
                  aria-hidden={!active}
                  className={`relative w-[min(82vw,680px)] shrink-0 select-none overflow-hidden rounded-[20px] border border-brand-dark bg-bg-card p-6 text-left shadow-[0_20px_55px_rgba(28,25,21,0.10)] transition-[opacity,transform] duration-500 [contain:paint] [will-change:transform,opacity] sm:p-9 ${active ? 'scale-100 opacity-100' : 'scale-[0.94] opacity-50'}`}
                >
