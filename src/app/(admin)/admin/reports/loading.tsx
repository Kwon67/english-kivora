...
function StatCardSkeleton() {
  return (
    <div className="rounded-[0.9rem] border border-border bg-card p-4 shadow-[var(--shadow-sm)] animate-pulse">
      ...
      <div className="mt-3 h-3 w-40 rounded bg-[var(--color-surface-hover)]" />
    </div>
  )
}
    ...
    <div className="space-y-6 animate-pulse pb-8">
      ...
      <div className="card overflow-hidden">
        <div className="border-b border-border px-5 py-4">
          ...
          <div className="mt-3 h-6 w-44 rounded bg-[var(--color-surface-hover)]" />
        </div>
        <div className="space-y-3 p-5">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="h-14 rounded-[0.9rem] bg-[var(--color-surface-hover)]" />
          ))}
        </div>
      </div>
        ...
        {Array.from({ length: 3 }).map((_, index) => (
          <div key={index} className="card p-5">
            ...
            <div className="mt-4 space-y-3">
              <div className="h-16 rounded-[0.9rem] bg-[var(--color-surface-hover)]" />
              <div className="h-16 rounded-[0.9rem] bg-[var(--color-surface-hover)]" />
            </div>
          </div>
        ))}
      ...
      <div className="card overflow-hidden">
        <div className="border-b border-border px-5 py-4">
          ...
          <div className="mt-3 h-6 w-40 rounded bg-[var(--color-surface-hover)]" />
        </div>
        <div className="h-72 bg-[var(--color-surface-container-low)]/40" />
      </div>
