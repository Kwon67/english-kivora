-- Add 'listening' to all game_mode / game_type CHECK constraints

-- 1. assignments.game_mode (original constraint from initial schema)
...
ALTER TABLE public.assignments ADD CONSTRAINT assignments_game_mode_check
  CHECK (game_mode IN ('multiple_choice', 'typing', 'flashcard', 'matching', 'listening', 'scheduled_review'));
...
ALTER TABLE public.assignment_templates ADD CONSTRAINT assignment_templates_game_mode_check
  CHECK (game_mode IN ('multiple_choice', 'typing', 'flashcard', 'matching', 'listening'));
...
ALTER TABLE public.arena_duels ADD CONSTRAINT arena_duels_game_type_check
  CHECK (game_type IN ('multiple_choice', 'matching', 'typing', 'flashcard', 'listening'));
